...
- **[DASHBOARD_6_PANELS_SCREENSHOT]:** docs/evidence/05-dashboard-6-panels.png
  ...
  Six panels (Latency P50/95/99, Traffic, Error rate + breakdown, Cost, Tokens in/out, Quality), each with units and an SLO threshold line. Two forms: a Markdown dashboard auto-generated by `scripts/generate_dashboard.py`, and a **professional live Chart.js dashboard** served at `GET /dashboard` (`app/dashboard.py`) — dark theme, dashed SLO lines, auto-refresh.
...
- **[BONUS_PRO_DASHBOARD]:** Professional live dashboard at `GET /dashboard` (`app/dashboard.py`) — 6 Chart.js panels (dark theme, dashed SLO lines, error/quality gauges, 5s auto-refresh, ~1h time series), same-origin with `/metrics` (no CORS).

  ![live dashboard](evidence/10-dashboard-live.png)
